...
namespace MediaControlDistributionCenter.Helpers.SocketClient
{
    ...
    public abstract class NetBase
    {
        ...
        public SocketState state = SocketState.Closed;
        ...
        public event Action<byte[]> ReceiveCompleted;
        public bool ReceiveSortData = true;
        ...
        private void ReceiveCallback(IAsyncResult ar)
        {
            try
            {
                ...
                //触发接收完成事件
                if (ReceiveSortData && ReceiveCompleted != null)
                {
                    ReceiveCompleted(byteBuffer);
                }

                //重新监听 数据接收
                if (socket != null)
                {
                    //数据量小一次性接收完成，清除之前的数据  为每次接收清空
                    if (ReceiveSortData)
                        Array.Clear(byteBuffer, 0, byteBuffer.Length); // 清空字节数组 
